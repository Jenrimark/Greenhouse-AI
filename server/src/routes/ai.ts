// AI 路由（阶段 1 A 任务：规则占位保留；阶段 2 替换为 ai-gateway / Agent runtime）
...
// 模拟面试：返回当前岗位的题目
router.post('/mock', validateBody(mockSchema), (req, res) => {
  const { oppId } = req.body
  const opp = db().opportunities.find((o) => o.id === oppId)
  if (!oppId || !opp) throw ApiError.notFound('机会不存在，请先在机会管线添加目标岗位')
  res.json({ questions: opp.questions ?? [], startedAt: new Date().toISOString() })
})
...
// 通用生成占位
router.post('/gen', (_req, res) => res.json({ ok: true, text: '' }))
router.post('/transcribe', (_req, res) => res.json({ text: '', segments: [] }))
router.post('/vision', (_req, res) => res.json({ ok: true }))
router.post('/resume-vision', (_req, res) => res.json({ ok: true }))

// 推荐 / 学分等
router.get('/referral', (_req, res) => res.json({ code: null }))
