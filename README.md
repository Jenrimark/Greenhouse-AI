...
> 说明：本项目用于本地学习与界面开发。原站的**编译样式、界面文案、岗位目录数据、插画与字体**
> 均从其线上构建产物中提取并原样保留；涉及大模型的能力（对话、简历生成、语音转写等）在本地以规则/占位实现替代。
...
| `/app/mock` | 模拟面试 | 选岗位/重点/风格/作答方式/题量，空态与原站一致 |
...
- **样式**：直接复用原站编译后的 `app.css`，React 组件严格沿用从其 JS 分包逆向出的原始类名；
  仅对未被打入主样式的懒加载屏（如简历工作室）与原生控件补了一个很小的 `overrides.css`。
- **文案**：从 i18n 分包枚举全部 key 并调用其翻译函数，导出完整中英字典，运行时按 `gr_lang` 切换。
- **数据**：岗位地图直接使用提取出的 `catalog.json`（43 行业 / 1443 岗位）与职级通道 `ladders.json`。
- **资源**：插画（`/art`）、字体、图标、品牌字标均为原站静态文件，放在 `client/public`。
- **头像**：原站依赖 DiceBear，无法直接移植，自写确定性 `PixelAvatar`（FNV-1a + mulberry32，对称像素小人）还原风格。
